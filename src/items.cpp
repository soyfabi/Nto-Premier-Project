...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"absorbpercentdoton", ITEM_PARSE_ABSORBPERCENTPOISON},
	{"absorbpercenthyoton", ITEM_PARSE_ABSORBPERCENTHYOTON},
		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			switch (parseType) {
				...
				case ITEM_PARSE_FIELDABSORBPERCENTENERGY: {
					abilities.fieldAbsorbPercent[combatTypeToIndex(COMBAT_RAITONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_FIELDABSORBPERCENTFIRE: {
					abilities.fieldAbsorbPercent[combatTypeToIndex(COMBAT_KATONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_FIELDABSORBPERCENTPOISON: {
					abilities.fieldAbsorbPercent[combatTypeToIndex(COMBAT_DOTONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_ABSORBPERCENTELEMENTS: {
					...
					abilities.absorbPercent[combatTypeToIndex(COMBAT_HYOTONDAMAGE)] += value;
					break;
				}
				...
				case ITEM_PARSE_ABSORBPERCENTHYOTON: {
					abilities.absorbPercent[combatTypeToIndex(COMBAT_HYOTONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_REFLECTPERCENTELEMENTS: {
					int16_t value = pugi::cast<int16_t>(valueAttribute.value());
					abilities.reflect[combatTypeToIndex(COMBAT_RAITONDAMAGE)].percent += value;
					abilities.reflect[combatTypeToIndex(COMBAT_KATONDAMAGE)].percent += value;
					abilities.reflect[combatTypeToIndex(COMBAT_DOTONDAMAGE)].percent += value;
					abilities.reflect[combatTypeToIndex(COMBAT_HYOTONDAMAGE)].percent += value;
					break;
				}
				...
				case ITEM_PARSE_REFLECTPERCENTMAGIC: {
					...
					abilities.reflect[combatTypeToIndex(COMBAT_DOTONDAMAGE)].percent += value;
					abilities.reflect[combatTypeToIndex(COMBAT_HYOTONDAMAGE)].percent += value;
					abilities.reflect[combatTypeToIndex(COMBAT_FUUTONDAMAGE)].percent += value;
					abilities.reflect[combatTypeToIndex(COMBAT_SUITONDAMAGE)].percent += value;
					break;
				}

				case ITEM_PARSE_REFLECTPERCENTENERGY: {
					abilities.reflect[combatTypeToIndex(COMBAT_RAITONDAMAGE)].percent += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTPERCENTFIRE: {
					abilities.reflect[combatTypeToIndex(COMBAT_KATONDAMAGE)].percent += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTPERCENTEARTH: {
					abilities.reflect[combatTypeToIndex(COMBAT_DOTONDAMAGE)].percent += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTPERCENTICE: {
					abilities.reflect[combatTypeToIndex(COMBAT_HYOTONDAMAGE)].percent += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTPERCENTHOLY: {
					abilities.reflect[combatTypeToIndex(COMBAT_FUUTONDAMAGE)].percent += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTPERCENTDEATH: {
					abilities.reflect[combatTypeToIndex(COMBAT_SUITONDAMAGE)].percent += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_REFLECTCHANCEELEMENTS: {
					int16_t value = pugi::cast<int16_t>(valueAttribute.value());
					abilities.reflect[combatTypeToIndex(COMBAT_RAITONDAMAGE)].chance += value;
					abilities.reflect[combatTypeToIndex(COMBAT_KATONDAMAGE)].chance += value;
					abilities.reflect[combatTypeToIndex(COMBAT_DOTONDAMAGE)].chance += value;
					abilities.reflect[combatTypeToIndex(COMBAT_HYOTONDAMAGE)].chance += value;
					break;
				}
				...
				case ITEM_PARSE_REFLECTCHANCEMAGIC: {
					...
					abilities.reflect[combatTypeToIndex(COMBAT_DOTONDAMAGE)].chance += value;
					abilities.reflect[combatTypeToIndex(COMBAT_HYOTONDAMAGE)].chance += value;
					abilities.reflect[combatTypeToIndex(COMBAT_FUUTONDAMAGE)].chance += value;
					abilities.reflect[combatTypeToIndex(COMBAT_SUITONDAMAGE)].chance += value;
					break;
				}

				case ITEM_PARSE_REFLECTCHANCEENERGY: {
					abilities.reflect[combatTypeToIndex(COMBAT_RAITONDAMAGE)].chance += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTCHANCEFIRE: {
					abilities.reflect[combatTypeToIndex(COMBAT_KATONDAMAGE)].chance += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTCHANCEEARTH: {
					abilities.reflect[combatTypeToIndex(COMBAT_DOTONDAMAGE)].chance += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTCHANCEICE: {
					abilities.reflect[combatTypeToIndex(COMBAT_HYOTONDAMAGE)].chance += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTCHANCEHOLY: {
					abilities.reflect[combatTypeToIndex(COMBAT_FUUTONDAMAGE)].chance += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_REFLECTCHANCEDEATH: {
					abilities.reflect[combatTypeToIndex(COMBAT_SUITONDAMAGE)].chance += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_BOOSTPERCENTELEMENTS: {
					int16_t value = pugi::cast<int16_t>(valueAttribute.value());
					abilities.boostPercent[combatTypeToIndex(COMBAT_RAITONDAMAGE)] += value;
					abilities.boostPercent[combatTypeToIndex(COMBAT_KATONDAMAGE)] += value;
					abilities.boostPercent[combatTypeToIndex(COMBAT_DOTONDAMAGE)] += value;
					abilities.boostPercent[combatTypeToIndex(COMBAT_HYOTONDAMAGE)] += value;
					break;
				}
				...
				case ITEM_PARSE_BOOSTPERCENTMAGIC: {
					...
					abilities.boostPercent[combatTypeToIndex(COMBAT_DOTONDAMAGE)] += value;
					abilities.boostPercent[combatTypeToIndex(COMBAT_HYOTONDAMAGE)] += value;
					abilities.boostPercent[combatTypeToIndex(COMBAT_FUUTONDAMAGE)] += value;
					abilities.boostPercent[combatTypeToIndex(COMBAT_SUITONDAMAGE)] += value;
					break;
				}

				case ITEM_PARSE_BOOSTPERCENTENERGY: {
					abilities.boostPercent[combatTypeToIndex(COMBAT_RAITONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_BOOSTPERCENTFIRE: {
					abilities.boostPercent[combatTypeToIndex(COMBAT_KATONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_BOOSTPERCENTEARTH: {
					abilities.boostPercent[combatTypeToIndex(COMBAT_DOTONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_BOOSTPERCENTICE: {
					abilities.boostPercent[combatTypeToIndex(COMBAT_HYOTONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_BOOSTPERCENTHOLY: {
					abilities.boostPercent[combatTypeToIndex(COMBAT_FUUTONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_BOOSTPERCENTDEATH: {
					abilities.boostPercent[combatTypeToIndex(COMBAT_SUITONDAMAGE)] += pugi::cast<int16_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_SUPPRESSENERGY: {
					if (valueAttribute.as_bool()) {
						abilities.conditionSuppressions |= CONDITION_RAITON;
					}
					break;
				}
				...
				case ITEM_PARSE_SUPPRESSFIRE: {
					if (valueAttribute.as_bool()) {
						abilities.conditionSuppressions |= CONDITION_KATON;
					}
					break;
				}
				...
				case ITEM_PARSE_FIELD: {
					...
					if (tmpStrValue == "katon") {
						conditionDamage = new ConditionDamage(CONDITIONID_COMBAT, CONDITION_KATON);
						combatType = COMBAT_KATONDAMAGE;
					} else if (tmpStrValue == "raiton") {
						conditionDamage = new ConditionDamage(CONDITIONID_COMBAT, CONDITION_RAITON);
				...
				case ITEM_PARSE_ELEMENTICE: {
					...
					abilities.elementType = COMBAT_HYOTONDAMAGE;
					break;
				}
