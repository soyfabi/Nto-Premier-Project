                          ...
                          uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count, Item* item, Cylinder* toCylinder)
{
	...
	if ((!player->hasFlag(PlayerFlag_CanMoveAllThings)) && (Position::getDistanceX(playerPos, mapToPos) > throwRange) || (Position::getDistanceY(playerPos, mapToPos) > throwRange)) {
		player->sendCancelMessage(RETURNVALUE_DESTINATIONOUTOFREACH);
		return;
	}

	if (!player->hasFlag(PlayerFlag_CanMoveAllThings) && !canThrowObjectTo(mapFromPos, mapToPos, true, false, throwRange, throwRange)) {
...
bool Game::playerSaySpell(Player* player, SpeakClasses type, const std::string& text)
{
	...
	if (result == TALKACTION_BREAK) {
		int32_t value;
        player->getStorageValue(g_config.getNumber(ConfigManager::STORAGEVALUE_EMOTE), value);
...
if (playersOnline >= 50 && playersOnline < 100) {
	rate += 1;
} else if (playersOnline >= 100 && playersOnline < 200) {
	rate += 2;
} else if (playersOnline >= 200 && playersOnline < 300) {
	rate += 3;
} else if (playersOnline >= 300 && playersOnline < 400) {
	rate += 4;
} else if (playersOnline >= 400 && playersOnline < 500) {
	rate += 5;
} else if (playersOnline >= 500) {
	rate += 6;
}
