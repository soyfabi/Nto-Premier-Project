...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			if ((attr = node.attribute("katon"))) {
				conditionType = CONDITION_KATON;
				...
				maxDamage = minDamage;
				tickInterval = 4000;
			} else if ((attr = node.attribute("raiton"))) {
			...
			combat->setParam(COMBAT_PARAM_TYPE, COMBAT_PHYSICALDAMAGE);
		} else if (tmpName == "poison" || tmpName == "doton") {
			...
			combat->setParam(COMBAT_PARAM_TYPE, COMBAT_HYOTONDAMAGE);
		...
		} else if (tmpName == "katoncondition" || tmpName == "raitoncondition" ||
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if ((attr = monsterNode.attribute("race"))) {
		...
		} else if (tmpStrValue == "katon" || tmpInt == 4) {
			mType->info.race = RACE_FIRE;
		} else if (tmpStrValue == "raiton" || tmpInt == 5) {
		...
		for (auto immunityNode : node.children()) {
			if ((attr = immunityNode.attribute("name"))) {
				...
				if (tmpStrValue == "physical") {
					...
					mType->info.conditionImmunities |= CONDITION_RAITON;
				} else if (tmpStrValue == "katon") {
					mType->info.damageImmunities |= COMBAT_KATONDAMAGE;
					mType->info.conditionImmunities |= CONDITION_KATON;
					...
					mType->info.damageImmunities |= COMBAT_HYOTONDAMAGE;
			...
			} else if ((attr = immunityNode.attribute("poison")) || (attr = immunityNode.attribute("doton"))) {
				if (attr.as_bool()) {
					mType->info.damageImmunities |= COMBAT_DOTONDAMAGE;
					mType->info.conditionImmunities |= CONDITION_POISON;
				}
			...
			} else if ((attr = immunityNode.attribute("hyoton"))) {
				if (attr.as_bool()) {
					mType->info.damageImmunities |= COMBAT_HYOTONDAMAGE;
					mType->info.conditionImmunities |= CONDITION_FREEZING;
				}
	...
	if ((node = monsterNode.child("elements"))) {
		for (auto elementNode : node.children()) {
			...
			} else if ((attr = elementNode.attribute("hyotonPercent"))) {
				mType->info.elementMap[COMBAT_HYOTONDAMAGE] = pugi::cast<int32_t>(attr.value());
				if (mType->info.damageImmunities & COMBAT_HYOTONDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"hyoton\" on immunity and element tags. " << file << std::endl;
				}
			} else if ((attr = elementNode.attribute("poisonPercent")) || (attr = elementNode.attribute("dotonPercent"))) {
