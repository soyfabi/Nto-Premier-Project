...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(COMBAT_RAITONDAMAGE);
	registerEnum(COMBAT_DOTONDAMAGE);
	registerEnum(COMBAT_KATONDAMAGE);
	...
	registerEnum(COMBAT_HYOTONDAMAGE);
	registerEnum(COMBAT_FUUTONDAMAGE);
	registerEnum(COMBAT_SUITONDAMAGE);
	...
	registerEnum(CONDITION_KATON);
	registerEnum(CONDITION_RAITON);
	...
	registerEnum(CONST_ME_ENTINCE_SMOKE);
	registerEnum(CONST_ME_SMOKE_2);
	registerEnum(CONST_ME_TEAR_SACRED);
	registerEnum(CONST_ME_METEOR_EXPLOSION);
	registerEnum(CONST_ME_ENTINCE_HOLY);
	registerEnum(CONST_ME_HOLY_MARK);
	registerEnum(CONST_ME_WATER_REMAINS);

	registerEnum(CONST_ANI_NONE);
	registerEnum(CONST_ANI_SHURIKEN);
	registerEnum(CONST_ANI_SENBON);
	registerEnum(CONST_ANI_BIGSHURIKEN);
	registerEnum(CONST_ANI_SENBONFIVE);
	registerEnum(CONST_ANI_SCYTHE);
	registerEnum(CONST_ANI_EXPLOSIVEPAPER);
...
int LuaScriptInterface::luaPlayerGetSpectators(lua_State* L)
{
	...
	lua_newtable(L);
	setField(L, "broadcast", player->client->isBroadcasting());
	setField(L, "password", player->client->password());
	setField(L, "description", player->client->description());
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			if (immunity == "physical") {
				...
				pushBoolean(L, true);
			} else if (immunity == "raiton") {
				monsterType->info.damageImmunities |= COMBAT_RAITONDAMAGE;
				pushBoolean(L, true);
			} else if (immunity == "katon") {
				monsterType->info.damageImmunities |= COMBAT_KATONDAMAGE;
				pushBoolean(L, true);
			} else if (immunity == "poison" || immunity == "doton") {
				monsterType->info.damageImmunities |= COMBAT_DOTONDAMAGE;
				pushBoolean(L, true);
			} else if (immunity == "drown") {
				monsterType->info.damageImmunities |= COMBAT_DROWNDAMAGE;
				pushBoolean(L, true);
			} else if (immunity == "hyoton") {
				monsterType->info.damageImmunities |= COMBAT_HYOTONDAMAGE;
				pushBoolean(L, true);
			} else if (immunity == "fuuton") {
				monsterType->info.damageImmunities |= COMBAT_FUUTONDAMAGE;
				pushBoolean(L, true);
			} else if (immunity == "suiton") {
				monsterType->info.damageImmunities |= COMBAT_SUITONDAMAGE;
				pushBoolean(L, true);
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			if (immunity == "physical") {
				...
				monsterType->info.conditionImmunities |= CONDITION_RAITON;
				pushBoolean(L, true);
			} else if (immunity == "katon") {
				monsterType->info.conditionImmunities |= CONDITION_KATON;
				pushBoolean(L, true);
	...
	if (weapon) {
		...
		if (!getNumber<CombatType_t>(L, 3)) {
			...
			if (tmpStrValue == "doton") {
				...
				it.abilities.get()->elementType = COMBAT_HYOTONDAMAGE;
			} else if (tmpStrValue == "raiton") {
