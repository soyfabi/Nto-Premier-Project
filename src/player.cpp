...
void Player::addSkillAdvance(skills_t skill, uint64_t count)
{
	...
	while ((skills[skill].tries + count) >= nextReqTries) {
		...
		sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("You have improved your {:s} and it is now level {:d}.", getSkillName(skill), skills[skill].level));
...
void Player::sendPing()
{
	...
	if (noPongTime >= noPongKickTime) {
		...
		g_game.removeCreature(this, true); //Fix Logout
	}
}
...
void Player::addManaSpent(uint64_t amount)
{
	...
	while ((manaSpent + amount) >= nextReqMana) {
		...
		sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("You have improved your ninjutsu, and it is now level {:d}.", magLevel));
